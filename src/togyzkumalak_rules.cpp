...
void ToguzEnv::reset() {
    ...
    kazans = {0, 0}; tuzduks = {-1, -1};
    to_play = PLAYER_1; steps = 0; winner_code = -2;
    ...
    if (stones == 1) {
        ...
        if (r == opp) {
            ...
            if (cnt == 3 && tuzduks[p] == -1 && c != 8 && tuzduks[opp] != c) {
                tuzduks[p] = c; kazans[p] += 3; board.set(target, 0);
            } else if (cnt % 2 == 0) {
                kazans[p] += cnt; board.set(target, 0);
            }
        }
    } else {
        board.set(start, 1);
        ...
        if (r == opp) {
            ...
            if (val == 3 && tuzduks[p] == -1 && c != 8 && tuzduks[opp] != c) {
                tuzduks[p] = c; kazans[p] += 3; board.set(last, 0);
            } else if (val % 2 == 0) {
                kazans[p] += val; board.set(last, 0);
            }
        }
    }
    steps++;
    if (kazans[p] >= WIN_THRESHOLD) winner_code = p;
    else if (kazans[opp] >= WIN_THRESHOLD) winner_code = opp;
    else if (steps >= max_steps) determine_winner_by_kazans();
    to_play = opp;
    if (winner_code != -2) return;
    std::array<int, 9> acts; update_legal_actions(acts);
    bool any = false; for (int i=0; i<9; ++i) if (acts[i]) any = true;
    if (!any) {
        for (int i=0; i<9; ++i) { kazans[0] += board.get(i); kazans[1] += board.get(i+9); board.set(i,0); board.set(i+9,0); }
...
std::vector<int> ToguzEnv::generate_moves() {
    std::array<int, 9> acts; update_legal_actions(acts);
    std::vector<int> res; for (int i=0; i<9; ++i) if (acts[i]) res.push_back(i);
    return res;
}

int ToguzEnv::get_random_move() {
    std::array<int, 9> acts; update_legal_actions(acts);
    bool ok = false; for(int i=0; i<9; ++i) if(acts[i]) ok=true;
    if (!ok) return -1;
    while(true) {
        int m = random_buffer[random_idx]; random_idx = (random_idx + 1) % 100000;
        if (acts[m]) return m;
    }
}
...
void ToguzEnv::determine_winner_by_kazans() {
    if (kazans[0] > kazans[1]) winner_code = 0; else if (kazans[1] > kazans[0]) winner_code = 1; else winner_code = -1;
}
...
void ToguzEnv::step_search(Bitboard& b, std::array<int, 2>& k, std::array<int, 2>& t,
                           int action, int turn, int& s_val, int max_s,
                           int& wc, int& n_play, bool& term, uint64_t& hash) {
    hash ^= Zobrist::to_play[turn];
    hash ^= Zobrist::kazan[0][k[0]]; hash ^= Zobrist::kazan[1][k[1]];
    hash ^= Zobrist::tuzduk[0][t[0]+1]; hash ^= Zobrist::tuzduk[1][t[1]+1];
    ...
    if (stones == 1) {
        ...
        if (tgt / 9 == opp) {
            int cnt = b.get(tgt); int col = tgt % 9;
            if (cnt == 3 && t[p] == -1 && col != 8 && t[opp] != col) {
                t[p] = col; k[p] += 3; hash ^= (opp == 0 ? Zobrist::board_p1[col][3] : Zobrist::board_p2[col][3]);
                b.set(tgt, 0); hash ^= (opp == 0 ? Zobrist::board_p1[col][0] : Zobrist::board_p2[col][0]);
            } else if (cnt % 2 == 0) {
                k[p] += cnt; hash ^= (opp == 0 ? Zobrist::board_p1[col][cnt] : Zobrist::board_p2[col][cnt]);
                b.set(tgt, 0); hash ^= (opp == 0 ? Zobrist::board_p1[col][0] : Zobrist::board_p2[col][0]);
            }
        }
    } else {
        ...
        if (last / 9 == opp) {
            ...
            if (val == 3 && t[p] == -1 && col != 8 && t[opp] != col) {
                t[p] = col; k[p] += 3; hash ^= (opp == 0 ? Zobrist::board_p1[col][3] : Zobrist::board_p2[col][3]);
                b.set(last, 0); hash ^= (opp == 0 ? Zobrist::board_p1[col][0] : Zobrist::board_p2[col][0]);
            } else if (val % 2 == 0) {
                k[p] += val; hash ^= (opp == 0 ? Zobrist::board_p1[col][val] : Zobrist::board_p2[col][val]);
                b.set(last, 0); hash ^= (opp == 0 ? Zobrist::board_p1[col][0] : Zobrist::board_p2[col][0]);
            }
        }
    }
    ...
    s_val++; wc = -2;
    if (k[p] >= WIN_THRESHOLD) wc = p; else if (k[opp] >= WIN_THRESHOLD) wc = opp;
    else if (s_val >= max_s) { if (k[0] > k[1]) wc = 0; else if (k[1] > k[0]) wc = 1; else wc = -1; }
    
    n_play = opp; term = (wc != -2);
    if (!term) {
...
int ToguzEnv::generate_moves_search(const Bitboard& b, const std::array<int, 2>& t, int player, std::array<int, 9>& out) {
    int count = 0, no = 1 - player;
    for (int i=0; i<9; ++i) if (b.get(player*9+i) > 0 && t[no] != i) out[count++] = i;
    return count;
}
...
double ToguzEnv::evaluate(const Bitboard& b, const std::array<int, 2>& k, const std::array<int, 2>& t,
                          int perspective, int to_play_idx, int wc, bool term) {
...
void ToguzEnv::render(const std::string& mode, const std::string& p1_name, const std::string& p2_name) const {
    if (mode != "terminal") return;
    std::cout << "Kazan: " << p1_name << "=" << kazans[0] << " " << p2_name << "=" << kazans[1] << "\n";
    
    // P2 pits: 17...9 (side2 indices 8...0)
    for (int i = 8; i >= 0; --i) {
        if (tuzduks[0] == i) std::cout << "[ X]";
        else std::cout << "[" << std::setw(2) << board.get(i + 9) << "]";
    }
    std::cout << "\n";
    
    // P1 pits: 0...8 (side1 indices 0...8)
    for (int i = 0; i < 9; ++i) {
        if (tuzduks[1] == i) std::cout << "[ X]";
        else std::cout << "[" << std::setw(2) << board.get(i) << "]";
    }
    std::cout << "\n";
}

int ToguzEnv::setup_random_position(int plies) {
    reset(); for (int i=0; i<plies; ++i) { if (is_game_over()) break; int m = get_random_move(); if (m == -1) break; step(m); }
    return steps;
}
