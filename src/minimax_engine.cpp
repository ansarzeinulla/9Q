...
bool deadline_reached(const Clock::time_point& deadline) { return Clock::now() >= deadline; }

double evaluate_leaf(const Bitboard& board, const std::array<int, 2>& kazans,
                     const std::array<int, 2>& tuzduks, int to_play_idx, int winner_code,
                     bool terminal, int perspective_player, const Evaluator& evaluator) {
  return evaluator.evaluate_position(
      BoardState{board, kazans, tuzduks, to_play_idx, winner_code, perspective_player, terminal});
}
...
void order_moves(std::array<int, 9>& moves, int count, const Bitboard& b, int player,
  ...
  for (int i = 0; i < count; ++i) {
    int move = moves[i];
    int score = (static_cast<uint8_t>(move) == tt_best_move)
                    ? 100000
                    : (1000 - move_order_key(b, player, move));
    scored[static_cast<size_t>(i)] = {move, score};
  }
  std::sort(scored.begin(), scored.begin() + count, [](const ScoredMove& a, const ScoredMove& b) {
...
double qsearch(Bitboard& board, std::array<int, 2>& kazans, std::array<int, 2>& tuzduks,
               int to_play_idx, int steps, int max_steps, int winner_code, double alpha,
               double beta, bool maximizing_player, int perspective_player, int q_depth,
               const Evaluator& evaluator) {
  if (winner_code != -2) {
    return evaluate_leaf(board, kazans, tuzduks, to_play_idx, winner_code, true, perspective_player,
                         evaluator);
  }

  double stand_pat = evaluate_leaf(board, kazans, tuzduks, to_play_idx, winner_code, false,
                                   perspective_player, evaluator);
  ...
  if (maximizing_player) {
    ...
    for (int i = 0; i < tac_count; ++i) {
      ...
      int ns = steps;
      uint64_t dummy = 0;
      ToguzEnv::step_search(board, kazans, tuzduks, tactical[i], to_play_idx, ns, max_steps, nw, np,
                            term, dummy);
      double res = qsearch(board, kazans, tuzduks, np, ns, max_steps, nw, alpha, beta, false,
                           perspective_player, q_depth - 1, evaluator);
      board = b_bak;
      kazans = k_bak;
      tuzduks = t_bak;
      to_play_idx = ot;
    ...
    for (int i = 0; i < tac_count; ++i) {
      ...
      ToguzEnv::step_search(board, kazans, tuzduks, tactical[i], to_play_idx, ns, max_steps, nw, np,
                            term, dummy);
      double res = qsearch(board, kazans, tuzduks, np, ns, max_steps, nw, alpha, beta, true,
                           perspective_player, q_depth - 1, evaluator);
      board = b_bak;
      kazans = k_bak;
      tuzduks = t_bak;
...
double minimax_raw(Bitboard& board, std::array<int, 2>& kazans, std::array<int, 2>& tuzduks,
  ...
  if (depth == 0) {
    return qsearch(board, kazans, tuzduks, to_play_idx, steps, max_steps, winner_code, alpha, beta,
                   maximizing_player, perspective_player, 6, evaluator);
  }

  auto& moves = history_moves[depth];
  int count = ToguzEnv::generate_moves_search(board, tuzduks, to_play_idx, moves);
  if (count == 0)
    return evaluate_leaf(board, kazans, tuzduks, to_play_idx, winner_code, false,
                         perspective_player, evaluator);
  ...
  if (maximizing_player) {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      ToguzEnv::step_search(board, kazans, tuzduks, moves[i], to_play_idx, ns, max_steps, nw, np,
                            term, next_h);
      double res = minimax_raw(board, kazans, tuzduks, np, ns, max_steps, nw, depth - 1, alpha,
                               beta, false, perspective_player, history_board, history_kazans,
                               history_tuzduks, history_moves, next_h, evaluator);
...
double qsearch_timed(Bitboard& board, std::array<int, 2>& kazans, std::array<int, 2>& tuzduks,
                     int to_play_idx, int steps, int max_steps, int winner_code, double alpha,
                     double beta, bool maximizing_player, int perspective_player, int q_depth,
                     const Evaluator& evaluator, const Clock::time_point& deadline,
                     bool& timed_out) {
  if (timed_out || deadline_reached(deadline)) {
    timed_out = true;
    return evaluate_leaf(board, kazans, tuzduks, to_play_idx, winner_code, winner_code != -2,
                         perspective_player, evaluator);
  }
  ...
  if (maximizing_player) {
    ...
    for (int i = 0; i < tac_count; ++i) {
      ...
      ToguzEnv::step_search(board, kazans, tuzduks, tactical[i], to_play_idx, ns, max_steps, nw, np,
                            term, dummy);
      double res = qsearch_timed(board, kazans, tuzduks, np, ns, max_steps, nw, alpha, beta, false,
                                 perspective_player, q_depth - 1, evaluator, deadline, timed_out);
      board = b_bak;
      kazans = k_bak;
      tuzduks = t_bak;
      to_play_idx = ot;
  ...
  for (int i = 0; i < tac_count; ++i) {
    ...
    ToguzEnv::step_search(board, kazans, tuzduks, tactical[i], to_play_idx, ns, max_steps, nw, np,
                          term, dummy);
    double res = qsearch_timed(board, kazans, tuzduks, np, ns, max_steps, nw, alpha, beta, true,
                               perspective_player, q_depth - 1, evaluator, deadline, timed_out);
    board = b_bak;
    kazans = k_bak;
    tuzduks = t_bak;
...
double minimax_raw_timed(Bitboard& board, std::array<int, 2>& kazans, std::array<int, 2>& tuzduks,
                         int to_play_idx, int steps, int max_steps, int winner_code, int depth,
                         double alpha, double beta, bool maximizing_player, int perspective_player,
                         std::vector<Bitboard>& history_board,
                         std::vector<std::array<int, 2>>& history_kazans,
  ...
  if (depth == 0) {
    return qsearch_timed(board, kazans, tuzduks, to_play_idx, steps, max_steps, winner_code, alpha,
                         beta, maximizing_player, perspective_player, 6, evaluator, deadline,
                         timed_out);
  }
  ...
  if (maximizing_player) {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      ToguzEnv::step_search(board, kazans, tuzduks, moves[i], to_play_idx, ns, max_steps, nw, np,
                            term, next_h);
      double res =
          minimax_raw_timed(board, kazans, tuzduks, np, ns, max_steps, nw, depth - 1, alpha, beta,
                            false, perspective_player, history_board, history_kazans,
                            history_tuzduks, history_moves, next_h, evaluator, deadline, timed_out);
      board = history_board[depth];
      kazans = history_kazans[depth];
      tuzduks = history_tuzduks[depth];
      to_play_idx = ot;
  ...
  for (int i = 0; i < count; ++i) {
    ...
    double res =
        minimax_raw_timed(board, kazans, tuzduks, np, ns, max_steps, nw, depth - 1, alpha, beta,
                          true, perspective_player, history_board, history_kazans, history_tuzduks,
                          history_moves, next_h, evaluator, deadline, timed_out);
    board = history_board[depth];
    kazans = history_kazans[depth];
    tuzduks = history_tuzduks[depth];
    to_play_idx = ot;
...
TimedMoveResult search_depth_timed(const ToguzEnv& env, int depth, const Evaluator& evaluator,
  ...
  for (int i = 0; i < count; ++i) {
    ...
    ToguzEnv::step_search(b_tmp, k_tmp, t_tmp, moves[i], env.to_play, ns, env.max_steps, nw, np,
                          term, next_h);
    double res = minimax_raw_timed(b_tmp, k_tmp, t_tmp, np, ns, env.max_steps, nw, depth - 1, alpha,
...
int get_best_move(const ToguzEnv& env, int depth, const Evaluator& evaluator) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    ToguzEnv::step_search(b_tmp, k_tmp, t_tmp, moves[i], env.to_play, ns, env.max_steps, nw, np,
                          term, next_h);
    double res = minimax_raw(b_tmp, k_tmp, t_tmp, np, ns, env.max_steps, nw, depth - 1, alpha, beta,
                             false, perspective, h_b, h_k, h_t, h_m, next_h, evaluator);
    if (res > best_val) {
      best_val = res;
      best_move = moves[i];
    }
...
TimedMoveResult get_best_move_timed(const ToguzEnv& env, double seconds_per_move, int max_depth,
  ...
  std::array<int, 9> moves;
  int count = ToguzEnv::generate_moves_search(env.board, env.tuzduks, env.to_play, moves);
  ...
  for (int depth = 1; depth <= max_depth; ++depth) {
    ...
    TimedMoveResult current = search_depth_timed(env, depth, evaluator, deadline);
