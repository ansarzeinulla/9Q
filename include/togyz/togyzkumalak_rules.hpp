...
constexpr int landing_pit(int start, int stones) {
  return (stones == 1) ? (start + 1) % BOARD_PITS : (start + stones - 1) % BOARD_PITS;
}
...
struct Bitboard {
  ...
  inline int get(int pit) const {
    if (pit < 9)
      return (int)((side1 >> (pit * 8)) & 0xFFU);
    else
      return (int)((side2 >> ((pit - 9) * 8)) & 0xFFU);
  }
  ...
  inline void add(int pit, int stones) {
    if (pit < 9)
      side1 += (uint128(stones) << (pit * 8));
    else
      side2 += (uint128(stones) << ((pit - 9) * 8));
  }
};
...
class ToguzEnv {
 public:
  ...
  void render(const std::string& mode, const std::string& p1_name = "P1",
              const std::string& p2_name = "P2") const;
  ...
  static double evaluate(const Bitboard& b, const std::array<int, 2>& k,
                         const std::array<int, 2>& t, int perspective, int to_play_idx, int w_code,
                         bool terminal);

  static void step_search(Bitboard& b, std::array<int, 2>& k, std::array<int, 2>& t, int action,
                          int to_play_idx, int& steps_val, int max_s, int& out_winner_code,
                          int& out_next_to_play, bool& out_is_terminal, uint64_t& current_hash);

  static int generate_moves_search(const Bitboard& b, const std::array<int, 2>& t, int player,
                                   std::array<int, 9>& out_moves);
};
